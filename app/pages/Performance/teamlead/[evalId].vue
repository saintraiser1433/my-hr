...
const departmentItems = computed(() => departments.value?.map((item) => ({
    id:item.id,
    label:item.title
})) || [])

const { data:departments, status, error } = await useAPI<DepartmentModel[]>("/department");
...
const viewRating = (employeeId:number) => {
...
watch(
  ...
  async (newId) => {
    ...
    try {
      const response = await $api<EmployeesEvaluate[]>(`/employees/evaluate/${newId}/${route.params.evalId}`);
...
<template>
    <PerformanceViewRatings v-model:open="isOpen" :title="title" :description="description"></PerformanceViewRatings>
    ...
    <div class="grid grid-cols-12 gap-2">
      <div class="col-span-4">
        <UCard :ui="{
          ...
          <UCommandPalette v-model="selectedDepartment" :groups="groups" class="flex-1" :ui="{item:'capitalize'}" />
        </UCard>
      </div>
