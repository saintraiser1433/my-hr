...
const proceedRepo = repository<InterviewDate>($api, "/interview");
const updateTime = async (data: InterviewDate) => {
  try {
    const response = await proceedRepo.update(data);
    ...
    ongoingData.value[0].progressList = ongoingData.value[0].progressList.map((item) =>
      item.id === data.id
        ? { ...item, dateInterview: new Date(data.date) }
        : item
    );
...
<template>
  ...
  <div class="flex flex-col lg:flex-row items-center gap-5 pb-2">
    <div class="flex items-center justify-center p-1 ml-5 border-1 bg-(--primary) rounded-full">
      <NuxtImg class="rounded-full" :src="`${config.public.STORAGE_URL_AVATAR}/${ongoingData[0]?.photo}`" quality="100"
      ...
      <div class="grid grid-cols-12 gap-5 ">
        <div class="col-span-12 lg:col-span-2">
          <div class="flex items-center gap-2">
            ...
            <h4>{{ ongoingData[0]?.appliedDate ? $datefns.format(new Date(ongoingData[0].appliedDate), "dd-MMM-yyyy") :
              "N/A" }}</h4>
  ...
  <ApplicantsProgress @data-date="updateTime" :data="ongoingData[0]?.progressList"> </ApplicantsProgress>
