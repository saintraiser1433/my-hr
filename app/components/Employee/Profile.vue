...
const triggerFileInput = () => {
    ...
    const nativeInput = fileInputRef.value?.$el?.querySelector('input[type="file"]');
    if (nativeInput) {
        nativeInput.click();
    }
...
const fileValue = useState<File | null>('employee-image',() => file.value);
...
<template>
<div class="flex gap-5 py-3 px-2 bg-(--sidebar-background) w-full shadow-lg border-l-3 border-(--ui-primary) rounded-md">
    <div class="relative p-1 ml-5 border-1 bg-(--primary) rounded-full w-42 h-42">
        <NuxtImg class="rounded-full" :src="previewUrl ? previewUrl :`${config.public.STORAGE_URL_AVATAR}/${information.photo_path}`" quality="100"></NuxtImg>
        <UInput ref="fileInputRef" type="file" @change="onFileChange" accept="image/*" class="hidden" size="sm" />
        <UButton icon="i-lucide-edit" class="absolute right-7 top-5 p-1 rounded-full shadow-lg" size="xs" variant="soft" color="success" @click="triggerFileInput"></UButton>
    </div>
