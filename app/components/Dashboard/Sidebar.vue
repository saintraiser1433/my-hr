<template>
  <aside :class="isToggle
    ...
    <footer class="flex relative items-center justify-center rounded-sm shadow-lg">
      <UDropdownMenu arrow class="w-full" :items="items" :content="{
        align: 'center',
        side: 'top',
      }" :ui="{
        ...
        <UButton class="flex justify-center" label="Open" color="neutral" variant="ghost">
          ...
          <div v-if="!isToggle" class="flex flex-col items-center justify-start px-2 flex-1">
            ...
            <span class="text-xs capitalize">{{ store.getRole }}</span>
          </div>
...
const fullname = computed(() => {
  if (store.getRole === "Admin") return "Administrator";
  return store.getFullName;
});
const sideBar = computed(() => {
  if (store.getRole === "Admin") return MENU_ITEMS;
  if (store.getRole === "Employee") return MENU_ITEMS_EMP;
  if (store.getRole === "TeamLead") return MENU_ITEMS_TEAM;

});

const goProfile = async () => {
  await navigateTo({ 'name': 'Employees-information-empId', params: { empId: store.getId } })
}
